export default async function teamRoutes(app) {
  ...
  app.get('/', async (req) => app.prisma.team.findMany({
    where: { workspaceId: req.user.workspaceId },
    orderBy: { name: 'asc' },
    include: { members: { include: { user: { select: { id: true, name: true, email: true } } } } },
  }));

  app.get('/mine', async (req) => app.prisma.team.findMany({
    where: { workspaceId: req.user.workspaceId, members: { some: { userId: req.user.id } } },
    orderBy: { name: 'asc' },
  ...
  app.post('/', async (req, reply) => {
    ...
    const team = await app.prisma.team.create({
      data: { workspaceId: req.user.workspaceId, name: name.trim(), color: color || '#3b82f6' },
    });
  ...
  app.patch('/:id', async (req, reply) => {
    if (!isOwner(req)) return reply.code(403).send({ error: 'owner_only' });
    const t = await app.prisma.team.findFirst({ where: { id: req.params.id, workspaceId: req.user.workspaceId } });
  ...
  app.delete('/:id', async (req, reply) => {
    ...
    const t = await app.prisma.team.findFirst({ where: { id: req.params.id, workspaceId: req.user.workspaceId } });
    if (!t) return reply.code(404).send({ error: 'not_found' });
    if (t.name === 'General') return reply.code(400).send({ error: 'cannot_delete_general' });
  ...
  app.post('/:id/members/:userId', async (req, reply) => {
    ...
    const u = await app.prisma.user.findFirst({ where: { id: req.params.userId, workspaceId: req.user.workspaceId } });
    ...
    await app.prisma.teamMember.upsert({
      ...
      update: {}, create: { teamId: t.id, userId: u.id },
    });
    return { ok: true };
  });
  ...
  app.delete('/:id/members/:userId', async (req, reply) => {
    ...
    const t = await app.prisma.team.findFirst({ where: { id: req.params.id, workspaceId: req.user.workspaceId } });
    if (!t) return reply.code(404).send({ error: 'team_not_found' });
    await app.prisma.teamMember.deleteMany({ where: { teamId: t.id, userId: req.params.userId } });
    return { ok: true };
  });
}
