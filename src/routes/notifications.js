...
export default async function notificationRoutes(app) {
  ...
  app.get('/unread-count', async (req) => {
    const count = await app.prisma.notification.count({ where: { userId: req.user.id, read: false } });
    return { count };
  });

  app.post('/mark-all-read', async (req) => {
    await app.prisma.notification.updateMany({
      where: { userId: req.user.id, read: false }, data: { read: true },
    });
    return { ok: true };
  });

  app.post('/:id/read', async (req, reply) => {
    const n = await app.prisma.notification.findFirst({ where: { id: req.params.id, userId: req.user.id } });
